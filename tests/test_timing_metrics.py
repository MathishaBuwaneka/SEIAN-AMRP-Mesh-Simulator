...
def test_batch_queue_overflow_records_one_attempt_and_one_drop_row() -> None:
    sim = chain_sim()
    initial_event_count = len(sim.packet_events)
    sim.config.queue_limit = 1
    source = sim.nodes["N01"]
    first = sim.create_packet(source, PacketType.GRID_STATE_UPDATE, destination_id="N02")
    second = sim.create_packet(source, PacketType.GRID_STATE_UPDATE, destination_id="N02")

    assert sim.route_packet("N01", first)
    assert not sim.route_packet("N01", second)

    assert sim.metrics.transmission_attempts == 2
    assert sim.metrics.packets_transmitted == 2
    assert sim.metrics.successful_link_transmissions == 2
    assert sim.metrics.drop_reasons["queue_overflow"] == 1
    assert len(sim.packet_events) == initial_event_count + 2
    assert sim.packet_events[-1]["delivered"] is False
    assert sim.packet_events[-1]["drop_reason"] == "queue_overflow"
