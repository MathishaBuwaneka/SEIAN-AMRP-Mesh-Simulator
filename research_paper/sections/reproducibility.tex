...
The merged upstream repository revision is \code{4a6f49d}. Local adapter
and validation changes made for this refresh are identified by SHA-256 hashes
in the provenance file; the upstream revision alone does not identify them.
Individual hashes identify the actual inputs used by the manuscript. The existing
...
Its case and build metadata are saved there. The refreshed matrix, GUI evidence,
merged-simulator replay, and separate raw outputs are under
\path{output/pscad_refresh_20260925} in the working checkout; these files are
ignored by Git, while the paper contains a compact hashed snapshot.
...
Run the artifact generator and then the PDF build from this folder:
...
The per-scenario raw outputs from this refresh are archived under
\path{output/pscad_refresh_20260925/raw} in the working checkout. The paper
package should be distributed with a stable raw-output archive for independent
reproduction.
