...
The six electrical experiments were rerun against the integrated repository
on 25 September 2026. Raw outputs were archived per condition outside the
manuscript package, while this paper freezes their result summaries and
figure previews. There is one saved deterministic result per condition in
this refresh, so no confidence interval,
...
Provenance includes the merged upstream revision, SHA-256 hashes of the
...
command edit through the dashboard.

The merged-simulator integration case is separate from the six-row matrix.
An N03 short-circuit-suspected event and an N05 communication-loss event are
injected in the merged networking simulator. Its event objects pass through
the domain-aware adapter; only the N03 event produces an isolation command.
A native N03 PSCAD fault is preprogrammed at 4.8--5.2 s and isolation at 5.0 s.
The networking simulator does not detect a PSCAD fault in a shared live run.
