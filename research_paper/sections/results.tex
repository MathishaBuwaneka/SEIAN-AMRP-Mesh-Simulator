...
All six newly rerun matrix cases report 31 research channels, zero build errors, and
...
\subsection{Merged Simulator and Dashboard Integration Evidence}
The additional merged-simulator replay carries two generated network fault
objects with explicit domains. The adapter emits one N03 power-isolation
command, three breaker transitions, and no N05 communication-fault breaker
command. A fresh 6.2-s PSCAD experiment records 124,001 samples on each of
31 channels, reports nine applied parameter writes, and returns no execution
error. The native fault was supplied separately in advance, so this case
verifies command compatibility and physical switching without claiming
fault-triggered closed-loop protection.

The refreshed graphical-validation artifact dated 25 September 2026 records three
...
connectivity, or a live feedback demonstration. The merged Python test suite
passed 318 tests after the adapter update, including the compiled packet-codec
checks. These software tests are not counted as electrical experiments.
