...
The networking simulator remains in its existing repository modules.
...
The integrated 25 September revision adds packet-driven decentralized route
learning, binary packet encoding, selectable LoRa airtime, a shared event-based
radio channel, and explicit communication versus electrical fault domains.
The current PSCAD interface consumes fault-event rows or explicit command
batches; it does not synchronize the radio event clock with the EMT solver.
For domain-aware fault rows, the adapter emits an electrical action only for
a power fault whose recommendation calls for switching. Communication and
device faults do not schedule breakers. Legacy rows without a domain retain
their earlier isolation behavior. The merged networking simulator's
\code{export_tables_json()} omits fault events, so the adapter receives its
\code{fault_events} collection explicitly.
